import { a as i, r as c, h as e, g as a } from './p-9031eb6a.js';
import { t as o } from './p-6a46bf1b.js';
import { l as t } from './p-d0805f56.js';
...
function s(i) {
  return r[i] ? r[i] : l.includes(i) ? `legacy/${i}` : i;
}
async function d(c) {
  const e = (function (i) {
    return i.replace('/', '-');
  ...
  const a = (async function (c) {
    const e = await fetch(i(`./icons/${c}.svg`));
    if (200 === e.status){
      const clearimage = await sanitizeSVG(e.text());
      return clearimage;
    }  
    throw new Error(`[gux-icon] fetching failed for icon "${c}" with status "${e.statusText} (${e.status})".`);
  })(c)
    .then(i => i)
    .catch(
      i => (
...
const p = class {
  constructor(i) {
    c(this, i), (this.iconName = void 0), (this.decorative = !1), (this.screenreaderText = ''), (this.svgHtml = void 0);
  }
  async prepIcon(i) {
    if (i) {
      const c = s(i);
      (this.baseSvgHtml = await d(c)), (this.svgHtml = this.getSvgWithAriaAttributes(this.baseSvgHtml));
    }
  }
  watchDecorative(i) {
    this.validateProps(i, this.screenreaderText),
      this.baseSvgHtml && (this.svgHtml = this.getSvgWithAriaAttributes(this.baseSvgHtml));
  }
  watchScreenreaderText(i) {
    this.validateProps(this.decorative, i),
      this.baseSvgHtml && (this.svgHtml = this.getSvgWithAriaAttributes(this.baseSvgHtml));
  }
  async componentWillLoad() {
    o(this.root, { variant: s(this.iconName) }), await this.prepIcon(this.iconName);
  }
  componentDidLoad() {
  ...
  getSvgWithAriaAttributes(i) {
    const c = new DOMParser().parseFromString(i, 'image/svg+xml').firstChild;

    return (
      c.setAttribute('aria-hidden', this.decorative ? String(this.decorative) : 'false'),
      this.screenreaderText ? c.setAttribute('aria-label', this.screenreaderText) : c.removeAttribute('aria-label'),
      c.outerHTML
    );
  }
  render() {
    return this.svgHtml && e('div', { class: 'gux-icon-container', innerHTML: this.svgHtml });
  }
  ...
  static get watchers() {
    return { iconName: ['prepIcon'], decorative: ['watchDecorative'], screenreaderText: ['watchScreenreaderText'] };
  }
};
p.style =
  ':host{display:inline-flex;width:1em;height:1em}.gux-icon-container{display:flex;align-items:center;justify-content:center;width:100%}.gux-icon-container svg{width:100%;max-width:100%;height:100%;max-height:100%;fill:currentcolor}';
export { p as gux_icon };
